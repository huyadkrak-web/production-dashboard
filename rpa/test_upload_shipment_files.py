...
if existing_shipment_files:
    ...
    print("출하파일 없음: 출하 파일 업로드는 건너뜁니다.")
...
with sync_playwright() as p:
    ...
    print("기준정보 불러오기")
    page.get_by_role("button", name="불러오기").nth(0).evaluate("button => button.click()")
    ...
    print("생산일보 계산 버튼 클릭")
    page.get_by_role("button", name="생산일보 계산").evaluate("button => button.click()")
    ...
    if existing_shipment_files:
        print("출하파일 선택")
        file_inputs.nth(3).set_input_files([str(file_path) for file_path in existing_shipment_files])
